/**
 * API Route: /api/data/btc
 * Endpoint para obtener y guardar datos históricos de Bitcoin
 * 
 * Métodos soportados:
...
import { INTERVAL_MAPPING, DATA_LIMITS, MESSAGES } from '@/config/constants';
...
/**
 * GET /api/data/btc
 * Obtiene datos de Bitcoin desde Binance y los guarda en la BD
 */
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    console.log(`\n🚀 [API] Procesando request para BTC (${interval}) - ${limit} velas`);

    // Obtener y guardar datos
    const result = await fetchAndSaveCryptoData('BTC', interval, limit);
